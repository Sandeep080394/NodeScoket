...
io.on('connection', function(socket) {
  ...
  logger.info('message', stringify({ socketconnected: socket });

  //------- start add user in connection ----------//
  socket.on('userlogin', loginInfo => {
    logger.info('message', { loginInfo: loginInfo });
    ...
    logger.info('message', { globalUsers: globalUsers });
  ...
  socket.on('chatinitiate', userinfo => {
    if (userinfo) {
      ...
      logger.info('message', { chatUsers: chatUsers });
  ...
  socket.on('sendmessage', data => {
    ...
    logger.info('message', { sendmessage: data });
    ...
    executeStoredProc('chatsave', data).then(res => {
      if (res) {
        logger.info('message', { chatsaved: res });
