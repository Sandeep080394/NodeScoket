...
io.on('connection', function(socket) {
  ...
  socket.on('chatinitiate', userinfo => {
    if (userinfo) {
      ...
      executeStoredProc('getchat', userinfo).then(
        res => {
          if (res && res.length > 0) {
            socket.emit('chatconnection', res);
          }
        },
        err => {
          socket.emit('chatconnection', err);
        }
      );
    }
  });
      ...
      executeStoredProc('commentreply', commentInfo).then(
        res => {
          ...
          if (res && res.length > 0 && res[0].length > 0) {
            ...
            commentResponseObj.CommentID=parseInt(commentResponseObj.CommentId);
            let commentReponse = {
