...
io.on('connection', function(socket) {
  ...
  socket.on('userlogin', loginInfo => {
    logger.log('message', { loginInfo: loginInfo });
    ...
    logger.log('message', { globalUsers: globalUsers });
  ...
  socket.on('chatinitiate', userinfo => {
    if (userinfo) {
      ...
      logger.log('message', { chatUsers: chatUsers });
  ...
  socket.on('sendmessage', data => {
    ...
    logger.log('message', { sendmessage: data });
    ...
    executeStoredProc('chatsave', data).then(res => {
      if (res) {
        logger.log('message', { chatsaved: res });
