...
io.on('connection', function(socket) {
  ...
  socket.on('commentconnect', function(trendInfo) {
    ...
    if (trendInfo) {
      var chatroom = 'comment-' + trendInfo.trendId;
      if (
        io.nsps['/'].adapter.rooms[chatroom] &&
        io.nsps['/'].adapter.rooms[chatroom].length > 1
      ) {
        socket.join(chatroom);
      }
      trendDataArr.push({ trendInfo: trendInfo, socket: socket });
      console.log('comment connect socket id', socket.id);
      // socket.broadcast
      //   .to(socket.id)
      //   .emit('myroom', { chatroom: chatroom, subscriptionId: socket.id });
  ...
  socket.on('sendcomment', function(commentInfo) {
    ...
    if (commentInfo) {
      ...
      socket.broadcast.to(commentInfo.chatroom).emit('commentdata', commentInfo.comment);
    }
  });
});
